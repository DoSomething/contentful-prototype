...
test('it can display a campaign update', () => {
  const wrapper = shallow(<ContentfulEntry json={{ id: '12345', type: 'campaignUpdate', fields: {} }} />);
  expect(wrapper.find('CampaignUpdateContainer')).toHaveLength(1);
});

test('it can display a CTA block', () => {
  const wrapper = shallow(<ContentfulEntry json={{ id: '12345', type: 'callToAction', fields: {} }} />);
  expect(wrapper.find('CallToActionContainer')).toHaveLength(1);
});

test('it can display a static block', () => {
  const wrapper = shallow(<ContentfulEntry json={{ id: '12345', type: 'static', fields: { content: 'Donec ullamcorper fringilla.', title: 'Nibh ornare' } }} />);
