...
export const pickWinner = (responses, questions) => {
  ...
  const finalTallies = Object.keys(responses).reduce(
    (currentTallies, questionId) => {
      ...
      awards.forEach(
        award => (newTallies[award] = (newTallies[award] || 0) + 1),
      );
  ...
  return Object.keys(finalTallies).sort(
    (alpha, beta) => finalTallies[beta] - finalTallies[alpha],
  )[0];
};
