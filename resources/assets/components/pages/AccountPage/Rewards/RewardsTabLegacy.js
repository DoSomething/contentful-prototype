...
const badgeCount = Number(query('badges')) || 0;
...
class RewardsTabLegacy extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <Query query={USER_BADGE_QUERY} variables={{ userId }}>
        {userData => (
          <div className="grid-wide bg-gray pb-6 wrapper">
            {featureFlag('rewards_levels') ? (
              <RewardsProgressBar
                badgesEarned={userData.user.badges}
...
RewardsTabLegacy.propTypes = {
  userId: PropTypes.string.isRequired,
};

export default RewardsTabLegacy;
